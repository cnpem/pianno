...
The annotation process consists of using the `pen` <span className='inline-flex items-baseline'><PenIcon className='self-center w-4 h-4'/></span> tool to mark pairs of points on the image that should be used to correct it's alignment.
...
[insert image of color picker]

## Creating an annotation

To create an annotation, the user must first select the `pen`<span className="inline-flex items-baseline"><PenIcon className="h-4 w-4 self-center" /></span>
...
Before saving, the user needs to erase points manually by selecting the `eraser`<span className="inline-flex items-baseline"><EraserIcon className="h-4 w-4 self-center" /></span> tool.
